...
import {    setCartCount, 
            ...
            setTotalPrice,
            setIsAdded,
            setIsAddedReset } from '../features/cart/cartSlice.js';
...
export const clearCart = () => async(dispatch) => {
    ...
    const url = 'http://52.78.206.153:9000/cart/clear';
    const data = {"id": id};
...
export const saveToCartList = (cartItem) => async(dispatch) => {
    const id = localStorage.getItem("user_id");
    const url = 'http://52.78.206.153:9000/cart/add';
...
export const deleteCartItem = (cid) => async(dispatch) => {
    const url = 'http://52.78.206.153:9000/cart/deleteItem';
    const data = {"cid": cid};
...
export const updateCartList = (cid, type) => async(dispatch) => {  
    const url = 'http://52.78.206.153:9000/cart/updateQty';
    const data = {"cid":cid, "type": type};
    
    const result = await axiosPut({url, data});  
    
    //수량이 업데이트 성공하면=> cartList 다시가져오기!!    
...
export const getCartList = () => async(dispatch) => {
    const id = localStorage.getItem("user_id");
    const url = 'http://52.78.206.153:9000/cart/items';
    const data = {"id":id};
...
export const getCount = () => async(dispatch) => {
    const id = localStorage.getItem("user_id");
    const url = 'http://52.78.206.153:9000/cart/count';
    const data = {"id":id};
